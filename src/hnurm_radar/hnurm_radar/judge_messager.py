...
class Receiver:
    def __init__(self ,my_color, shared_is_activating_double_effect , shared_enemy_health_list , shared_enemy_marked_process_list , shared_have_double_effect_times , shared_time_left , shared_dart_target):
    ...
    def log_buffer_content(self, buffer):
        # print(f"Buffer content: {buffer.hex()}")
        pass
        ...
        while True:
            ...
            while True:
                ...
                try:
                    # 检查缓冲区中是否有足够的数据来解析帧头
                    if len(buffer) < 5:
                        # print("buffer is less than 5, real size: {}".format(len(buffer)))
                        break
                    ...
                    if len(buffer) < sof_index + 5:
                        # print("buffer is less than sof_index + 5, real size: {}".format(len(buffer)))
                        break

                    header = buffer[sof_index:sof_index + 5]
                    data_length, seq, crc8 = struct.unpack('<HBB', header[1:])
                    # print("data_length {} iseq {}".format(data_length, seq))
                    ...
                    if len(buffer) < sof_index + frame_length:
                        # print("buffer is less than sof_index + frame_length({}), buffer size: {}".format(
    ...
    def switch_method(self, cmd_id, data):
        ...
        # print(f"find command id{cmd_id_value}")
        try:
...
class JudgeMessager(Node):
    def __init__(self):
        ...
        # 读取配置信息
        self.cfg = YAML().load(open("/home/rm/lsa/radar/hnurm_radar/configs/main_config.yaml", encoding='Utf-8', mode='r'))
        ...
        self.receiver = Receiver(self.my_color, self.shared_is_activating_double_effect , self.shared_enemy_health_list , self.shared_enemy_marked_process_list , self.shared_have_double_effect_times , self.shared_time_left , self.shared_dart_target)
        self.receiver.start()
        
        # 打开串口
        self.get_logger().info('Trying to Open Serial port!')
        self.ser = serial.Serial('/dev/ttyACM0', 115200, timeout=1)  # 根据实际情况修改串口
    ...
    def send_double_effect(self):
        ...
        # self.get_logger().info(str(self.my_id))
        ...
        while rclpy.ok():
            ...
            for i in range(len(cur_locations.locs)):
                ...
                if location.label == 'Red': # 此时检测到机器人都是红方，代表自己是蓝方
                    ...
                    # self.get_logger().info('Red robot {}: x = {}, y = {}'.format(location.id, x, y))
                else:
                   
                    
                    
                    # self.get_logger().info('Blue robot {}: x = {}, y = {}'.format(location.id, x, y))
